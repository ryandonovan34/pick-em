...
Most background work runs via APScheduler (`app/services/scheduler.py`), starting in-process when the app boots (`main.py`'s `lifespan`) and stopping on shutdown. **Jobs live in memory only** — nothing is persisted, so a redeploy wipes any pending job. Note that a Fly.io machine *suspend/resume* (see `fly.toml`'s `auto_stop_machines`) is **not** a redeploy: the process is checkpointed and thawed rather than restarted, so `lifespan`'s startup code — and therefore any "reschedule on boot" logic — does not run again on resume. A job that's only rearmed at real process boot effectively stops firing once the machine starts suspending between requests, silently rather than with an error.

The startup ingest and 24h odds-refresh interval job are subject to this — they only truly self-heal across a genuine redeploy/restart, not a suspend/resume cycle. Results-fetching used to be a third in-process interval job with the exact same weakness; it's now driven externally instead (`POST /admin/results/fetch`, called on a schedule by `.github/workflows/results-fetch.yml`), which sidesteps the problem entirely since the trigger lives outside the app process and its own HTTP call wakes a suspended machine on demand. The two **per-game** jobs (pick reminders and the per-game pre-kickoff refresh/lock) have a different, milder issue: they're scheduled once, tied to a specific future `run_date`, with no re-hydration logic on startup — if the process restarts between when one was scheduled and its fire time, that specific job is simply lost. A pick reminder is recovered the next time that slate changes (which reschedules the whole week); a missed spread lock just means the line stays whatever it last was, unlocked, until the game's own results are processed.
...
| Results polling | Externally triggered (`POST /admin/results/fetch`) — every 30 min on NFL game days, once/day otherwise, per `.github/workflows/results-fetch.yml` | Queries the Odds API scores endpoint, but only for the real sport key(s) actually needed — if no slate game with a past kickoff is still pending a result, nothing is queried at all. Note: the endpoint only returns scores for games completed within the last 3 days (`daysFrom` caps at 3) — a game that goes more than 3 days without this job running needs a one-off manual backfill. |
